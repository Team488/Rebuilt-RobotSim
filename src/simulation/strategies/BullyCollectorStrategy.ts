...
import { findBestEVBall, findNearestEmptyTile } from "../StrategyUtils";
...
export class BullyCollectorStrategy extends InactiveScoringStrategy {
  ...
  actionTime = 0.5;
  description =
    "Prioritizes collecting balls that are near opponent robots to disrupt their paths.";
  isDelivering = false;
  patience = 0;
  ...
  decideMove(robot: Robot, field: Field): { x: number; y: number } | null {
    ...
    const opponents = (field.engine?.robots || []).filter(
      (r: any) => r.team === opponentTeam,
    );
    ...
    const { ball: targetBall } = findBestEVBall(
      ...
      (_, c) => {
        ...
        const boundary =
          robot.team === "RED" ? FIELD_WIDTH * 0.6 : FIELD_WIDTH * 0.4;
