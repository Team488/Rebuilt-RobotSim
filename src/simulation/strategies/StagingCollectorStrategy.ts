        ...
        if (this.isDelivering) {
            ...
            if (stagingLoc) {
                ...
                if (dist <= robot.maxShootDistance && dist > 1.5) {
                    return { type: "SHOOT", distance: dist, angle: Math.atan2(stagingLoc.y + 0.5 - robot.y, stagingLoc.x + 0.5 - robot.x) };
                }
            ...
            if (r >= 0 && r < field.grid.length && c >= 0 && c < field.grid[0].length) {
                if (field.grid[r][c] === FieldTile.BALL && robot.ballCount < robot.maxBalls) {
                    return { type: "COLLECT" };
                }
            }
        }
